...
public class PullDrawable extends Drawable {
    private static final String TAG = "PullDrawable";
    private Paint mPaint;// 外部边框
    private Paint mInnerPaint;// 内部矩形和线条

    private Path mBorderPath;
    private Path mBorderDstPath;// 外边框path
    ...
    private void initPath(Rect bounds) {
        ...
        mBorderPathMeasure = new PathMeasure(mBorderPath, true);// 路径测量类，回路
        ////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void draw(Canvas canvas) {
        Log.d(TAG, "draw");
        canvas.drawPath(mBorderDstPath, mPaint);// 画外部边框圆角矩形
        canvas.drawPath(mRectDstPath, mInnerPaint);// 左上角：矩形
        canvas.drawPath(mLine1DstPath, mInnerPaint);// 右上角：线1
        canvas.drawPath(mLine2DstPath, mInnerPaint);// 右上角：线2
        canvas.drawPath(mLine3DstPath, mInnerPaint);// 右上角：线3
        canvas.drawPath(mLine4DstPath, mInnerPaint);//低部：线1
        canvas.drawPath(mLine5DstPath, mInnerPaint);//低部：线2
        canvas.drawPath(mLine6DstPath, mInnerPaint);//低部：线3
    }
    ...
    public void update(float percent) {
        ...
        mBorderPathMeasure.getSegment(0, mBorderPathMeasure.getLength() * percent, mBorderDstPath, true);// 逆时针方向
